...
import com.cesela.gmall.util.JwtUtil;
...
public class PassportController {
    ...
    public String login(UmsMember umsMember, HttpServletRequest request){
        ...
        // 调用用户服务验证用户名和密码
        UmsMember umsMemberLogin = umsService.login(umsMember);
        ...
        if(umsMemberLogin!=null){
            ...
            // 将token存入redis一份
            umsService.addUserToken(token,memberId);
        }else{
            // 登录失败
            token = "fail";
        }
    ...
    public String vlogin(String code, HttpServletRequest request) {
        ...
        paramMap.put("code",code);// 授权有效期内可以使用，没新生成一次授权码，说明用户对第三方数据进
        ...
        UmsMember umsMemberCheck = umsService.checkOauthUser(umsCheck);
        if (umsMemberCheck == null) {
            umsMember = umsService.addOauthUser(umsMember); // 注意mybatis的主键返回策略不能跨rpc使用
        } else {
